...
Rearranging again will give the following equation, also known as the Generalized Method of Moments

\begin{equation}
\varepsilon = P - C + \Omega(P, X \mid \theta)^{-1} Q(P, X \mid \theta)
\end{equation}

The equation below signifies that, when evaluated at the true parameter values $\theta^{*}$, the expected cost residual $\varepsilon$, after accounting for the instruments ${Z}$, must be zero. This relies on the instruments being uncorrelated with the unobservable cost shocks $\varepsilon$ but correlated with the demand and cost variables they are intended to influence.
...
This is no different than any of the minimization problems that we have worked with in our class, the only difference being that we want the argument, $\hat{\theta}$ that minimizes the objective. Conceptually, we want to find the parameters so that the predicted cost shocks generated by the structural model are orthogonal to demographic variation. The moment conditions complete the bridge between the observed data, the theoretical model, and the equilibrium pricing behavior that the paper seeks to quantify.
